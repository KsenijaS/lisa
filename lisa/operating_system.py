...
class Debian(Linux):
    ...
    def _get_os_version(self) -> OsVersion:
        os_version = OsVersion("")
        # Output of 'lsb_release -as' doesn't match the pattern.
        # No LSB modules are available.
        # Debian
        # Debian GNU/Linux 10 (buster)
        # 10
        # buster
        # Change it into 'lsb_release -ar'.
        cmd_result = self._node.execute(
            cmd="lsb_release -ar", shell=True, no_error_log=True
        )
        if cmd_result.exit_code == 0 and cmd_result.stdout != "":
            for row in cmd_result.stdout.splitlines():
            ...
            raise LisaException(
                f"Command 'lsb_release -ar' failed. "
                f"exit_code:{cmd_result.exit_code} stderr: {cmd_result.stderr}"
            )
