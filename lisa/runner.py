...
class RootRunner(Action):
    ...
    def _fetch_runners(self) -> Iterator[BaseRunner]:
        ...
        if root_runbook.combinator:
            ...
            return self._generate_runners(root_runbook)
    ...
    def _start_loop(self) -> None:
        ...
        try:
            remaining_runners = [next(runner_iterator)]
        except StopIteration:
            self._log.debug("no runner found")
        if self._runners:
            ...
            while task_manager.wait_worker() or remaining_runners:
                ...
                if not remaining_runners and has_more_runner:
                    # fetch runners, if existing runners completed
                    try:
